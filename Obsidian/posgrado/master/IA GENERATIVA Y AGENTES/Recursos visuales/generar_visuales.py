...
# 11 Transformer decoder-only
fig,ax=canvas('Transformer decoder-only: del prefijo a la distribución','La atención mezcla contexto; la red feed-forward transforma cada posición.',size=(14,6))
ax.set_xlim(0,14)
box(ax,.1,3.4,1.5,1,'Texto');box(ax,2,3.4,1.7,1,'Tokens e IDs');arrow(ax,(1.6,3.9),(2,3.9))
box(ax,4.1,3.4,2,1,'Embedding\n+ posición',BLUE);arrow(ax,(3.7,3.9),(4.1,3.9))
box(ax,6.7,4.1,2.1,.9,'Atención causal',TEAL);box(ax,6.7,2.7,2.1,.9,'Red feed-forward',TEAL)
arrow(ax,(6.1,3.9),(6.7,4.5));arrow(ax,(7.75,4.1),(7.75,3.6))
ax.add_patch(FancyBboxPatch((6.35,2.35),2.8,3.05,boxstyle='round,pad=0.08,rounding_size=.16',facecolor=TEAL+'08',edgecolor=TEAL,lw=1.4,linestyle='--'))
ax.text(7.75,2.5,'bloque × N',ha='center',fontsize=10,color=TEAL)
box(ax,9.65,3.4,1.6,1,'LM head',ORANGE);arrow(ax,(8.8,3.15),(9.65,3.7))
box(ax,11.55,3.4,1.2,1,'Softmax',ORANGE);arrow(ax,(11.25,3.9),(11.55,3.9))
ax.text(6.6,.85,'La máscara impide mirar el futuro. El resultado es P(siguiente token | prefijo).',ha='center',fontsize=13)
save(fig,'11-transformer-decoder')
# 12 Attention matrix
scores=np.array([[1.0,np.nan,np.nan,np.nan],[.25,.75,np.nan,np.nan],[.15,.25,.60,np.nan],[.10,.20,.30,.40]])
fig,ax=plt.subplots(figsize=(8,6),layout='constrained');fig.suptitle('Atención causal: cada fila combina solo el pasado',fontsize=18,fontweight='bold')
masked=np.ma.masked_invalid(scores);cmap=plt.get_cmap('Blues').copy();cmap.set_bad('#e2e8f0')
im=ax.imshow(masked,vmin=0,vmax=1,cmap=cmap)
labs=['El','gato','duerme','hoy'];ax.set(xticks=range(4),yticks=range(4),xticklabels=labs,yticklabels=labs,xlabel='Key / Value disponible',ylabel='Query que se está calculando')
for i in range(4):
 for j in range(4):
  ax.text(j,i,'futuro' if np.isnan(scores[i,j]) else f'{scores[i,j]:.2f}',ha='center',va='center',color='white' if not np.isnan(scores[i,j]) and scores[i,j]>.5 else INK,fontsize=11)
fig.colorbar(im,ax=ax,label='Peso de atención')
ax.text(0,-.15,'Cada fila suma 1 entre las posiciones permitidas.',transform=ax.transAxes,fontsize=10,color=GRAY)
save(fig,'12-atencion-causal')
# 13 Decoding controls
tokens=np.array(['A','B','C','D','E']);logits=np.array([4.0,2.2,1.0,.4,0.0])
fig,axes=plt.subplots(1,3,figsize=(14,4.8),layout='constrained');fig.suptitle('Decodificación: tres maneras de transformar la misma salida',fontsize=18,fontweight='bold')
for T,c in [(.5,ORANGE),(1,BLUE),(2,TEAL)]:
 p=np.exp(logits/T-np.max(logits/T));p=p/p.sum();axes[0].plot(tokens,p,marker='o',lw=2,label=f'T = {T}',color=c)
axes[0].set(title='Temperatura',ylabel='Probabilidad',ylim=(0,1));axes[0].legend(fontsize=9);axes[0].grid(alpha=.15)
p=np.exp(logits-np.max(logits));p=p/p.sum();topk=np.argsort(p)[-3:];pk=np.zeros_like(p);pk[topk]=p[topk];pk=pk/pk.sum()
axes[1].bar(tokens,pk,color=[BLUE if x>0 else '#cbd5e1' for x in pk]);axes[1].set(title='Top-k = 3',ylim=(0,1));axes[1].text(.5,.92,'Dos candidatos quedan en 0',transform=axes[1].transAxes,ha='center',fontsize=10,color=GRAY)
order=np.argsort(p)[::-1];cum=np.cumsum(p[order]);cut=np.searchsorted(cum,.9)+1;keep=set(order[:cut]);pp=np.array([v if i in keep else 0 for i,v in enumerate(p)]);pp=pp/pp.sum()
axes[2].bar(tokens,pp,color=[TEAL if x>0 else '#cbd5e1' for x in pp]);axes[2].set(title='Top-p = 0.9',ylim=(0,1));axes[2].text(.5,.92,f'El núcleo conserva {cut} tokens',transform=axes[2].transAxes,ha='center',fontsize=10,color=GRAY)
for ax in axes:ax.set_xlabel('Token candidato')
fig.text(.5,-.02,'Ejemplo didáctico: T redistribuye masa; top-k y top-p eliminan candidatos y renormalizan.',ha='center',fontsize=11,color=GRAY)
save(fig,'13-decodificacion')
# 14 Model lifecycle and alignment
fig,ax=canvas('Ciclo de vida de un modelo fundacional','Cada etapa cambia la señal de aprendizaje; inferencia no actualiza los pesos.',size=(14,7))
box(ax,.2,4.1,2.1,1.1,'Preentrenamiento\ntexto sin etiquetar',BLUE)
box(ax,3.1,4.1,2.1,1.1,'SFT\ndemostraciones',TEAL);arrow(ax,(2.3,4.65),(3.1,4.65))
box(ax,6,4.1,2.1,1.1,'Preferencias\npares o principios',ORANGE);arrow(ax,(5.2,4.65),(6,4.65))
box(ax,9.9,4.1,2,1.1,'Inferencia\nprompt + muestreo',BLUE);arrow(ax,(8.1,4.65),(9.9,4.65))
for x,txt,c in [(4.8,'RLHF\nrecompensa + PPO',ORANGE),(7.0,'DPO\npérdida directa',TEAL),(9.2,'Constitutional AI\ncrítica + principios',BLUE)]:box(ax,x,1.55,2.1,1,txt,c)
arrow(ax,(7.05,4.05),(5.85,2.55));arrow(ax,(7.05,4.05),(8.05,2.55));arrow(ax,(7.05,4.05),(10.25,2.55))
ax.text(7.2,.55,'RLHF, DPO y Constitutional AI son alternativas o combinaciones del ajuste por preferencias.',ha='center',fontsize=12)
save(fig,'14-ciclo-alineamiento')
# 15 Experimental measurement pipeline
fig,ax=canvas('Comparar modelos: medir antes de decidir','El mismo problema, los mismos casos y el mismo prompt para todos.',size=(14,6))
ax.set_xlim(0,14)
box(ax,.1,3.4,2,1.2,'10 casos\n+ respuestas esperadas',BLUE)
box(ax,2.8,3.4,1.9,1.2,'Mismo prompt\ny configuración',BLUE);arrow(ax,(2.1,4),(2.8,4))
for y,lab,c in [(4.7,'Modelo A',ORANGE),(3.4,'Modelo B',TEAL),(2.1,'Modelo C',BLUE)]:box(ax,5.5,y,1.8,.8,lab,c);arrow(ax,(4.7,4),(5.5,y+.4))
box(ax,8.0,3.4,1.8,1.2,'Resultados\ncrudos');
for y in [5.1,3.8,2.5]:arrow(ax,(7.3,y),(8,4),rad=.08)
box(ax,10.5,4.25,2.1,.9,'Exactitud\nverificador',TEAL);box(ax,10.5,2.9,2.1,.9,'Latencia\ncronómetro',ORANGE);box(ax,10.5,1.55,2.1,.9,'Costo\ntokens × precio',BLUE)
for y in [4.7,3.35,2.0]:arrow(ax,(9.8,4),(10.5,y))
ax.text(6.5,.55,'La decisión incorpora además privacidad, hardware, estabilidad y restricciones de producción.',ha='center',fontsize=12)
save(fig,'15-comparacion-modelos')
print('15 figuras generadas en PNG y SVG.')
